// ignore_for_file: non_constant_identifier_names, unused_local_variable, unnecessary_brace_in_string_interps, unnecessary_string_interpolations, constant_identifier_names
...
import "package:main/update_tdlib/update_tdlib.dart" as update_tdlib;
  ...
  Future<Map?> userbot({
    ...
    tg.on(tg.event_invoke, (UpdateTd update) async {
      if (update.raw["@type"] == "error") {
        if (update.raw["message"] is String) {
          if (RegExp(r"(api_id|api_hash|telegram\.org)", caseSensitive: false)
              .hasMatch(update.raw["message"])) {
            logger.err(
                "Set Tdlib Parameters Gagal\npastikan api id dan api hash benar ya");
            exit(0);
          }
        }
        ...
        if (update.raw["@type"] == "updateAuthorizationState") {
          if (update.raw["authorization_state"] is Map) {
            ...
            if (tg.client_id != update.client_id) {
              ...
              Map? res = await tg.initClient(update,
    ...
    for (var i = 0; i < name_clients.length; i++) {
      ...
      if (i == 0) {
        ...
        await tg.initIsolateNewClient(
          clientId: tg.td_create_client_id(),
          clientOption: {
            "name_client": name_client,
            'database_directory':
                path.join(telegram_directory.path, "${name_client}"),
